...
async function runHarshShader(GPUdevice, headerArray, targetArray, wgs_x, wgs_y, wgs_z, dwg_x, dwg_y, dwg_z, itercount, totalThread, isTestMode = false, wgslIter) {
    ...
    const shaderModule = device.createShaderModule({
        code: wgslCode(dispatchSize, workgroupSize, itercount, totalThread, isTestMode, wgslIter)
    });
...
const wgslCode = (dispatchSize, workgroupSize, iterCount, totalThread, isTestMode, wgslIter) => {
...
fn main(@builtin(global_invocation_id) global_invocation_id : vec3<u32>) {
    ...
    for(var i:u32 = 0; i<${wgslIter}; i++){
        let nonce : u32 = i + u32(${baseNonceOffset}) + 
